/**
 * 此模組增加對 Arcgis 的支援
 * 因為套件本身內建對ARCGIS的轉換功能之執行環境並不適用於Nodejs
 * 目前僅僅將其移植過來
 * 
 * 有必要的話可以用TS改寫、以利擴充ARCGSI的特殊幾何轉換功能
*/
...
/**
 * Ues IIFE Promise for binding the Arcgis support function to the prototype of WKT
 */
export default (async ()=>{

    let Circle:__esri.CircleConstructor = await loadModule("esri/geometry/Circle")
    // let Mesh:__esri.geometry.Mesh = await loadModule("esri/geometry/Mesh")
    let Multipoint:__esri.MultipointConstructor = await loadModule("esri/geometry/Multipoint")
    let Point:__esri.PointConstructor = await loadModule("esri/geometry/Point")
    let Polygon:__esri.PolygonConstructor = await loadModule("esri/geometry/Polygon")
    let Polyline:__esri.PolylineConstructor = await loadModule("esri/geometry/Polyline")
        ...
        return new Polygon({
            ...
            rings: (function() {
                ...
                rings = that.components.map(function(i) {
                    ...
                    /**
                     * 2020/01 含有洞之MUTI多邊形既有錯誤
                     * @see https://github.com/arthur-e/Wicket/blob/master/wicket-arcgis.js#L207
                     */
 ...
 * (ring2) argument
 * @param   ring1   {Array} An Array of vertices that describe a ring in an Polygon instance
 * @param   ring2   {Array} An Array of vertices that describe a ring in an Polygon instance
...
Wkt.isInnerRingOf = function(ring1, ring2, srs) {
    ...
    for (i = 0; i < ring1.length; i += 1) {
        ...
        pnt = new Point({x:ring1[i].x,y:ring1[i].y,spatialReference:srs});
...
Wkt.Wkt.prototype.deconstruct = function(obj) {
    ...
    // Polygon ///////////////////////////////////////////////////
    if (obj.constructor === Polygon || obj.constructor === Circle) {
